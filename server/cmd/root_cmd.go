...
import (
	...
	"github.com/koki/kubernetes-viewer/server/logging"
...
func envFromEnv() Env {
	...
	if len(env.CookieAuthenticationKey) == 0 {
		log.Fatalf("missing %s", envCookieAuthKey)
	}

	if len(env.StripeKey) == 0 {
		log.Fatalf("missing %s", envStripeKey)
	}

	if len(env.GithubClientID) == 0 {
		log.Fatalf("missing %s", envGithubClientID)
	}

	if len(env.GithubClientSecret) == 0 {
		log.Fatalf("missing %s", envGithubClientSecret)
	}
...
func init() {
	...
	// parse the go default flagset to get flags for all packages
